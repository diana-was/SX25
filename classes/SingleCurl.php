...
/*	APPLICATION:	PrincetonIT API
	FILE:			Singlecurl.php
	DESCRIPTION:	Class to connect via Curl 
 ...
 */
 class SingleCurl_Class {
     ...
     * @return void
     */
     public function __construct($url='',$timeOut = 5,$acceptType='text/html',$verb='GET',$includeHeader = false, $followlocation = true,$maxRedirecs = 4,$binaryTransfer = false,$noBody = false)
     {
		$this->_url 			= $url;
		...
		$this->_proxy = !empty($config->proxy)?(is_array($config->proxy)?$config->proxy:array($config->proxy)):array();
     }
     ...
     */
     public function setPost ($requestBody)
     {
     	if ($requestBody == null)
     		$requestBody = array();
        $this->_post = true;
        $this->_postFields = $this->buildPostBody($requestBody);
     }
     ...
     * @return void
     */
	public function createCurl($verb,$url=null,$requestBody=null,$useProxy=true)
     {
        $this->__flush();
     	$headers = array();
		...
		$this->setAuth();
         ...
         if ($useProxy)
         {
         	...
         	{
				$t = 0;   //rand(0, count($this->_proxy) - 1);    // use first proxy
				$proxies 	= is_array($this->_proxy)?explode(":",$this->_proxy[$t]):explode(":",$this->_proxy);
         	...
         	if (empty($server))
			{
				// take the mark out of the url if no proxy
				$this->opt(CURLOPT_URL				, str_replace('&CUT_XYZ59','',$this->_url));
			}
         }
